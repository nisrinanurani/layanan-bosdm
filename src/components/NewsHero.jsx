...
import { ArrowRight, Info, Megaphone, ChevronLeft, ChevronRight } from 'lucide-react'; // Tambah Chevron Icon
...
export default function NewsHero() {
    ...
    // Auto Slide (Reset timer kalau user klik manual agar tidak bentrok)
    useEffect(() => {
        const timer = setInterval(() => {
            nextSlide();
        }, 6000);
        return () => clearInterval(timer);
    }, [nextSlide]); // Dependency ke nextSlide
    ...
    return (
        <section className="relative w-full h-[85vh] min-h-[600px] overflow-hidden bg-blue-50 group">
            ...
            {/* === 2. NAVIGATION ARROWS (Floating) === */}
            {/* Tombol Kiri */}
            <button
                onClick={prevSlide}
                className="absolute left-4 top-1/2 -translate-y-1/2 z-30 p-3 rounded-full bg-white/30 backdrop-blur-md border border-white/50 text-slate-700 hover:bg-white hover:text-blue-600 hover:scale-110 transition-all shadow-lg opacity-0 group-hover:opacity-100 duration-300"
            >
            ...
            <button
                onClick={nextSlide}
                className="absolute right-4 top-1/2 -translate-y-1/2 z-30 p-3 rounded-full bg-white/30 backdrop-blur-md border border-white/50 text-slate-700 hover:bg-white hover:text-blue-600 hover:scale-110 transition-all shadow-lg opacity-0 group-hover:opacity-100 duration-300"
            >
                ...
                <div className="max-w-7xl mx-auto px-6 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-end lg:items-center">
                    ...
                    <div className="lg:col-span-7 relative z-20">
                        ...
                        >
                            ...
                            <h1 className="text-4xl md:text-6xl font-serif font-bold leading-tight mb-4 text-slate-900 drop-shadow-sm">
                            ...
                            <div className="flex flex-wrap gap-4">
                                <button
                                    ...
                                    className="px-8 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors shadow-lg shadow-blue-100"
                                >
                                    Baca Selengkapnya
                                </button>
                            </div>

                            {/* Indikator Slide (Dots) - Tambahan manis di bawah tombol */}
                            <div className="flex gap-2 mt-8">
                                {NEWS_DATA.map((_, idx) => (
                                    <button
                                        key={idx}
                                        onClick={() => setCurrentIndex(idx)}
                                        className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentIndex ? 'w-8 bg-blue-600' : 'w-2 bg-blue-300 hover:bg-blue-400'
                                            }`}
                                    />
                                ))}
                            </div>
                    ...
                    {/* --- KANAN: FLYER INFO (SOLID WHITE) --- */}
                    <div className="hidden lg:block lg:col-span-5">
                        <motion.div
                            initial={{ opacity: 0, x: 50 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ duration: 0.8, delay: 0.5 }}
                            className="bg-white p-8 rounded-2xl shadow-2xl shadow-blue-100/50 border border-blue-50 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300"
                        >
                            <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50 rounded-bl-[100px] -z-0" />

                            <div className="flex flex-col gap-4 relative z-10">
                                <div className="flex items-center gap-3 mb-2">
                                    <div className="p-2.5 bg-blue-100 rounded-lg shrink-0 text-blue-700">
                                        <Info className="w-6 h-6" />
                                    </div>
                                    <span className="text-xs font-extrabold tracking-widest text-blue-700 uppercase bg-blue-50 px-2 py-1 rounded">
                                        Info Terbaru
                                    </span>
                                </div>

                                <div>
                                    <h3 className="text-2xl font-bold text-slate-900 mb-3 font-serif leading-tight">
                                        {FLYER_INFO.title}
                                    </h3>
                                    <p className="text-sm text-slate-600 font-medium leading-relaxed mb-6">
                                        {FLYER_INFO.desc}
                                    </p>
                                    <a href="#" className="inline-flex items-center px-5 py-2.5 bg-blue-700 text-white text-sm font-bold rounded-lg hover:bg-blue-800 transition-colors gap-2 shadow-md">
            ...
            <div className="absolute bottom-0 left-0 right-0 z-20 bg-blue-50 border-t border-blue-200 h-14 flex items-center shadow-sm">
                ...
                <div className="overflow-hidden whitespace-nowrap w-full relative">
                    ...
                    >
                        {[...NEWS_DATA, ...NEWS_DATA, ...NEWS_DATA, ...NEWS_DATA].map((news, i) => (
                            <div key={i} className="flex items-center gap-3 text-sm text-slate-600 group cursor-pointer hover:text-blue-800 transition-colors">
                                <span className="w-2 h-2 rounded-full bg-blue-400 shrink-0 group-hover:bg-blue-600 transition-all"></span>
                                <span className="font-semibold tracking-wide">{news.title}</span>
                                <span className="text-slate-400 mx-2">|</span>
                                <span className="text-blue-600 text-xs uppercase tracking-wider">{news.category}</span>
                            </div>
                        ))}
                    </motion.div>
                    <div className="absolute top-0 right-0 h-full w-24 bg-gradient-to-l from-blue-50 to-transparent pointer-events-none z-20" />
                </div>
            </div>
