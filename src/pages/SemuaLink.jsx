...
import {
    ...
    ExternalLink, Save, PlusCircle, Globe, ArrowLeft, AlertCircle
} from 'lucide-react';
...
export default function SemuaLink({ userRole }) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans pb-20">
            {/* === NAVBAR (SERAGAM DENGAN PROFIL BIRO) === */}
            ...
            <main className="max-w-6xl mx-auto py-12 px-6">
                ...
                {/* FILTER & SEARCH BAR (MUNCUL UNTUK SEMUA ROLE) */}
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
                    <div className="relative w-full max-w-lg">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 transition-all shadow-inner font-medium"
                            value={searchLink}
                ...
                {showModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl p-8 overflow-hidden">
                            <h3 className="text-2xl font-black mb-6 text-slate-900">{newLinkData.id ? "Edit Kumpulan Link" : "Tambah Kumpulan Link"}</h3>
                            <div className="space-y-6">
                                <div>
                                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Judul Kumpulan</label>
                                    <input type="text" placeholder="Contoh: Portal Absensi" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 transition-all" value={newLinkData.judul} onChange={(e) => setNewLinkData({ ...newLinkData, judul: e.target.value })} />
                                </div>
                                <div>
                                    ...
                                    <div className="space-y-3 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar">
                                        {newLinkData.links.map((link, idx) => (
                                            <div key={idx} className="flex gap-2">
                                                <div className="flex-1 bg-slate-50 border border-slate-200 px-4 py-3 rounded-2xl flex items-center gap-3"><LinkIcon className="w-4 h-4 text-blue-500" /><input type="text" placeholder="https://..." className="text-sm outline-none bg-transparent w-full font-medium text-blue-600" value={link} onChange={(e) => handleUpdateLinkValue(idx, e.target.value)} /></div>
                                                {idx > 0 && <button onClick={() => handleRemoveField(idx)} className="p-3 text-red-500 hover:bg-red-50 rounded-xl transition-colors"><Trash2 className="w-4 h-4" /></button>}
                                            </div>
                                        ))}
                                    </div>
                                    <button onClick={handleAddField} className="mt-4 flex items-center gap-2 text-blue-600 font-bold text-xs hover:underline"><PlusCircle className="w-4 h-4" /> Tambah Input Link</button>
                                </div>
                            </div>
                            <div className="flex gap-3 mt-10">
                                ...
                                <button onClick={handleSaveLinks} className="flex-[2] py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-lg hover:bg-blue-700 transition-all flex items-center justify-center gap-2"><Save className="w-5 h-5" /> Simpan Perubahan</button>
                            </div>
                        </motion.div>
                    </div>
                )}
