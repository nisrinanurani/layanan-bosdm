...
export default function BeritaKami({ userRole }) {
    ...
    // Logic Toggle Hero dengan Checkbox & Limit 10
    const handleToggleHero = (id) => {
        ...
        const updatedData = data.map(item => {
            if (item.id === id) {
                ...
                // Jika ingin menceklis (mengaktifkan) tapi sudah 10
                if (!item.isHero && currentHeroCount >= 10) {
                    alert("⚠️ Batas Maksimal Tercapai! Hanya 10 konten yang boleh tampil di Beranda.");
                    return item;
                }
                return { ...item, isHero: !item.isHero };
            }
            return item;
        });
            ...
            <div className="max-w-[1600px] mx-auto">
                ...
                <header className="flex justify-between items-center mb-12">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-xl font-black text-brand-dark uppercase tracking-tighter">Pusat Manajemen Konten</h2>
                    </div>
                ...
                <section className="mb-16">
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-sm font-black uppercase tracking-widest text-brand-dark">Preview Slot Hero ({currentHeroCount}/10)</h2>
                        </div>
                        {/* CATATAN PENTING */}
                        <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 px-4 py-2 rounded-xl">
                            ...
                            <p className="text-[10px] font-black text-amber-700 uppercase tracking-tight">
                                Catatan: Hanya 10 konten pilihan yang akan ditayangkan di NewsHero halaman depan.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-6 scrollbar-hide">
                        {heroItems.length > 0 ? heroItems.map((hero) => (
                            ...
                            <div className="w-full py-10 border-2 border-dashed border-brand-gray-200 rounded-[2.5rem] flex items-center justify-center bg-white/50">
                                <p className="text-brand-gray-300 font-bold uppercase text-[10px] tracking-[0.3em]">Gunakan ceklis di bawah untuk memilih konten Hero</p>
                            </div>
                        )}
                    </div>
                </section>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-center gap-6 mb-10">
                    ...
                    {isAdmin && (
                        <button onClick={() => navigate('/berita-kami/editor')} className="w-full md:w-auto bg-brand-primary text-white px-10 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-brand-blue-100 hover:bg-brand-dark transition-all flex items-center justify-center gap-3">
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-brand-gray-50 bg-brand-gray-50/20">
                                {isAdmin && <th className="p-8 font-black text-[11px] uppercase text-brand-gray-300 text-center">Tampilkan</th>}
                                <th className="p-8 font-black text-[11px] uppercase tracking-widest text-brand-gray-300">Detail Konten</th>
                        ...
                        <tbody className="divide-y divide-brand-gray-50">
                            {data.filter(i => i.judul.toLowerCase().includes(searchTerm.toLowerCase())).map((item) => (
                                <tr key={item.id} className={`hover:bg-brand-blue-50/20 transition-all group ${item.isHero ? 'bg-brand-blue-50/10' : ''}`}>
                                    {isAdmin && (
                                        <td className="p-8 text-center">
                                            <input
                                                ...
                                                className="w-6 h-6 rounded-lg border-2 border-brand-gray-200 text-brand-primary focus:ring-brand-primary cursor-pointer disabled:cursor-not-allowed disabled:opacity-30"
                                            />
                                        </td>
                                    )}
                                    <td className="p-8">
                                        <div className="flex items-center gap-6">
                                            <img src={item.tipe === 'BERITA' ? item.gambar : item.flyer} className="w-20 h-14 rounded-xl object-cover shadow-sm bg-brand-dark" alt="thumb" />
                                            <div>
                                                <div className="font-black text-brand-dark uppercase text-sm group-hover:text-brand-primary transition-colors mb-1">{item.judul}</div>
