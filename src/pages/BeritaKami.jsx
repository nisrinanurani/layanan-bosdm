...
export default function BeritaKami({ userRole }) {
    ...
    const filteredData = data.filter(item => {
        ...
        if (filterTanggal) {
            ...
            matchesDate = itemDate === filterTanggal;
        }
            ...
            <div className="max-w-[1600px] mx-auto">
                ...
                <section className="mb-12">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 px-4 py-2 rounded-xl">
                            ...
                            <p className="text-[9px] font-black text-amber-700 uppercase tracking-tight">Maksimal 10 konten pilihan untuk slide utama.</p>
                        </div>
                    </div>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-center gap-6 mb-10">
                    <div className="flex flex-col md:flex-row items-center gap-4 w-full lg:w-auto">
                        ...
                        {/* --- FILTER KOTAK GABUNGAN (KALENDER & KATEGORI) --- */}
                        <div className="flex items-center bg-white border border-slate-200 rounded-[1.2rem] shadow-sm overflow-visible w-full md:w-auto">
                            ...
                            {/* Filter Tanggal (Browser Native Date Picker wrapped nicely) */}
                            <div className="relative flex items-center gap-4 px-6 py-4 border-r border-slate-100 group">
                                <input
                                    type="date"
                                ...
                                />
                                <span className={`text-sm font-bold ${filterTanggal ? 'text-blue-600' : 'text-slate-400'}`}>
                                    {filterTanggal ? new Date(filterTanggal).toLocaleDateString('id-ID') : 'dd/mm/yyyy'}
                                </span>
                                <CalendarIcon className="w-5 h-5 text-slate-300 group-hover:text-blue-600 transition-colors" />
                            </div>
                            ...
                            <div className="relative" ref={dropdownRef}>
                                <div
                                    ...
                                    className="px-8 py-4 flex items-center gap-10 cursor-pointer hover:bg-slate-50 transition-colors min-w-[200px] justify-between"
                                >
                                    <span className="text-slate-400 text-sm font-bold">{filterTipe}</span>
                                    <ChevronDown className={`w-5 h-5 text-slate-300 transition-transform ${showKategoriDropdown ? 'rotate-180' : ''}`} />
                                </div>

                                {showKategoriDropdown && (
                                    <div className="absolute top-full left-0 mt-2 w-full bg-white border border-slate-100 rounded-2xl shadow-xl z-50 overflow-hidden py-2">
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {filteredData.length > 0 ? filteredData.map((item) => (
                                        ...
                                        <td className="p-8 text-center">
                                            <div className="flex justify-center">
                                                <label className="relative flex items-center cursor-pointer">
                                                    ...
                                                    <div className={`w-7 h-7 border-2 rounded-lg transition-all flex items-center justify-center ${item.isHero ? 'bg-blue-600 border-blue-600' : 'bg-white border-slate-200'}`}>
                                    ...
                                    <td className="p-8">
                                        <div className="flex items-center gap-6">
                                            <div className="w-24 h-16 rounded-xl overflow-hidden bg-slate-100 shrink-0 shadow-sm">
                                                <img src={item.tipe === 'BERITA' ? item.gambar : item.flyer} className="w-full h-full object-cover" alt="" />
                                            </div>
                                            <div>
                                                ...
                                                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{new Date(item.waktu).toLocaleDateString('id-ID')}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-8 text-center">
                                        <span className={`px-4 py-1.5 rounded-lg font-black text-[9px] uppercase ${item.tipe === 'BERITA' ? 'bg-blue-100 text-blue-600' : 'bg-indigo-100 text-indigo-600'}`}>
                                            {item.tipe}
                                        </span>
                                    </td>
                                    <td className="p-8">
                                        <div className="flex justify-center gap-6 items-center">
                                            <button onClick={() => navigate(`/berita/${item.id}`)}><Eye className="w-5 h-5 text-slate-200 hover:text-blue-600 transition-colors" /></button>
                                            {isAdmin && (
                                                <>
                                                    <button onClick={() => navigate(`/berita-kami/editor/${item.id}`)}><Edit3 className="w-5 h-5 text-slate-200 hover:text-amber-500 transition-colors" /></button>
                                                    <button onClick={() => handleDelete(item.id)}><Trash2 className="w-5 h-5 text-slate-200 hover:text-red-500 transition-colors" /></button>
                                                </>
                                            )}
                                        </div>
                                    </td>
                                ...
                                <tr><td colSpan={isAdmin ? 4 : 3} className="p-20 text-center text-slate-300 font-bold uppercase text-xs italic tracking-widest">Tidak ada konten ditemukan</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
