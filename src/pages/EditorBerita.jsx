...
export default function EditorBerita({ userRole }) {
    ...
    return (
        <div className="min-h-screen bg-[#F0F7FF] pb-20">
            <nav className="border-b bg-[#F0F7FF]/90 backdrop-blur-md px-8 py-5 sticky top-0 z-50 flex justify-between items-center">
            ...
            <main className="max-w-5xl mx-auto mt-10 px-6">
                {/* PILIHAN TIPE */}
                <div className="flex gap-4 mb-10 bg-[#E1EFFF] p-2 rounded-3xl w-fit shadow-sm border border-[#8ECAFE]/30">
                    <button onClick={() => setPost({ ...post, tipe: 'BERITA' })} className={`px-8 py-4 rounded-2xl flex items-center gap-3 font-black text-[10px] tracking-widest transition-all ${post.tipe === 'BERITA' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-[#F0F7FF]'}`}>
                        <FileText className="w-4 h-4" /> UNGGAH BERITA
                    </button>
                    <button onClick={() => setPost({ ...post, tipe: 'FLYER' })} className={`px-8 py-4 rounded-2xl flex items-center gap-3 font-black text-[10px] tracking-widest transition-all ${post.tipe === 'FLYER' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-[#F0F7FF]'}`}>
                        <ImageIcon className="w-4 h-4" /> UNGGAH FLYER
                    </button>
                </div>

                <div className="bg-[#E1EFFF] rounded-[3.5rem] p-12 shadow-2xl border border-[#8ECAFE]/20">
                    ...
                    {post.tipe === 'BERITA' ? (
                        ...
                        <div className="text-center py-20 border-4 border-dashed border-[#8ECAFE]/40 rounded-[3rem] bg-[#F0F7FF]">
                            {post.flyer ? (
