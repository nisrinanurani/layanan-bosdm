    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20">
            ...
            <main className="max-w-5xl mx-auto mt-10 px-6">
                {/* PILIHAN TIPE */}
                <div className="flex gap-4 mb-10 bg-white p-2 rounded-3xl w-fit shadow-sm border border-slate-100">
                    <button onClick={() => setPost({ ...post, tipe: 'BERITA' })} className={`px-8 py-4 rounded-2xl flex items-center gap-3 font-black text-[10px] tracking-widest transition-all ${post.tipe === 'BERITA' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}>
                        <FileText className="w-4 h-4" /> UNGGAH BERITA
                    </button>
                    <button onClick={() => setPost({ ...post, tipe: 'FLYER' })} className={`px-8 py-4 rounded-2xl flex items-center gap-3 font-black text-[10px] tracking-widest transition-all ${post.tipe === 'FLYER' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}>
                        <ImageIcon className="w-4 h-4" /> UNGGAH FLYER
                    </button>
                </div>

                <div className="bg-white rounded-[3.5rem] p-12 shadow-2xl border border-slate-50">
